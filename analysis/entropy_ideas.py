...
def monte_carlo_distribution(generating_model, model_list, hparam_list,
                             function, samples=DEFAULT_SAMPLES):
    ...
    for model, hparams in zip(model_list, hparam_list):
        nll_list.append(
            compute_nll(sample_dataset, model, hparams)
        )
...
def monte_carlo_entropy(model, hparams, samples=DEFAULT_SAMPLES):
    ...
    return monte_carlo_distribution(model,
                                    [model], [hparams],
                                    lambda nlls: nlls[0],
                                    samples=samples)


def monte_carlo_cross_entropy(model_1, model_2, model2_hparams, samples=DEFAULT_SAMPLES):
    """Performs a monte carlo estimate of the cross-entropy between model1 and model2."""
    return monte_carlo_distribution(model_1,
                                    [model_2], [model2_hparams],
                                    lambda nlls: nlls[0],
                                    samples=samples)


def monte_carlo_KL(model_1, model_2, model_1_hparams, model_2_hparams, samples=DEFAULT_SAMPLES):
    """Performs a monte carlo estimate of the KL divergence between model1 and model2"""
    return monte_carlo_distribution(model_1,
                                    [model_1, model_2], [model_1_hparams, model_2_hparams],
