...
def fit_sklearn_unsupervised(
    ...
    def get_rejection_rate(data, model):
        ...
        rejection_rate = (
            1 - np.mean(prediction)
        ) / 2  # as the prediction is +1 for accept and -1 for reject
    ...
    ood_rejection_rates = [
        get_rejection_rate(ood_norms, sklearn_model)
        for ood_norms in normed_ood_data_list
    ]
...
def rejection_rate_table(model_names, dataset_names, get_data, ModelClass, **params):
    ...
    for model_name, id_dataset in zip(model_names, dataset_names):
        ...
        (
            id_fit_rejection_rate,
            id_test_rejection_rate,
            ood_rejection_rates,
        ) = fit_sklearn_unsupervised(id_data, all_data_list, sklearn_model, **params)

        rejection_table[model_name] = [
            id_fit_rejection_rate,
            id_test_rejection_rate,
        ] + ood_rejection_rates
...
def norm_rejection_rate_table(
    batch_size, model_names, dataset_names, ModelClass, **params
):
    def get_norm_data(model_name, id_dataset):
        id_norms, all_norms_list = get_norms(
            batch_size, model_name, id_dataset, dataset_names
        )

        return get_sklearn_norms(id_norms, all_norms_list)

    return rejection_rate_table(
        model_names, dataset_names, get_norm_data, ModelClass, **params
    )
...
def raw_image_rejection_rate_table(batch_size, dataset_names, ModelClass, **params):
    ...
    return rejection_rate_table(
        dataset_names, dataset_names, get_image_data, ModelClass, **params
    )
...
if __name__ == "__main__":
    ...
    for batch_size in [32, 10, 5, 1]:
        ...
        rrt = norm_rejection_rate_table(
            batch_size,
            model_names,
            dataset_names,
            IsolationForest,
            fit_sample_proportion=0.6,
            n_estimators=1000,
        )
    ...
    for batch_size in [32, 10, 5, 1]:
        ...
        rrt = raw_image_rejection_rate_table(
            batch_size,
            dataset_names,
            IsolationForest,
            fit_sample_proportion=0.6,
